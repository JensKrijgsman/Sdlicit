...
All fields are read once at `Orchestrator` construction time (project init). Changing one requires initialising the project again (`POST /api/v1/init`, or restarting the CLI session against the same `.sdlicit/config.yaml` after editing it) rather than a live toggle mid session, consistent with these being setup time ablation choices in the validation framework, not runtime user preferences.

## Diagrams

Generated automatically from AST analysis, no code execution needed. Regenerate with `pylint`'s `pyreverse` and [`py-code-visualizer`](https://pypi.org/project/py-code-visualizer/) (both `dev` group dependencies); Graphviz is needed for SVG output.
