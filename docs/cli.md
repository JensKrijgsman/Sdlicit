...
- **Composing** — create a new ADR (step-by-step wizard with live agent suggestions and Socratic follow-ups), list/view ADRs, suggest ADR directions.
...
`stages/composing/create_adr.py` runs a split-pane wizard: fields on the left, live agent suggestions on the right, updated as background threads return. If a new ADR's title looks similar to an existing one (server-side Jaccard title overlap), the CLI surfaces a suggestion and offers to mark the old ADR as superseded on save (`client.supersede_adr`), which removes the old ADR's knowledge base chunks and ingests the new one in their place — the file on disk is left for you to annotate.
...
There is no free-form chat surface in the CLI (it is a structured wizard, not a chat UI), so the extension's automatic Socratic-suggestion loop over an open conversation has no CLI equivalent yet.
