...
| **CLI (TUI)** | [`cli/`](cli/) | A `rich`-based terminal client. Pure presentation layer — talks to the backend exclusively over REST, no direct imports of the `sdlicit` package. |
| **VS Code extension (GUI)** | [`extension/`](extension/) | The graphical frontend: artifact panels, a BDD/trace-coverage explorer, a guided-flow wizard, and a knowledge-base browser, all backed by the same REST API. |
...
This provisions Python 3.12, `uv`, and Node 22, and (once a `.venv` exists) auto-patches a couple of NixOS-specific wheel-linking quirks. Without Nix, just make sure you have Python 3.12+ and [`uv`](https://docs.astral.sh/uv/) installed.
...
Then open the `extension/` folder in VS Code and press `F5` to launch an Extension Development Host, or package it with `vsce package` and install the `.vsix`. The extension auto-starts the backend server for you (`sdlicit.autoStartServer` setting) if one isn't already running.
