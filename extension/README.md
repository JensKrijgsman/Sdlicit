...
- **Knowledge Base** — the ingested corpus (standards, briefs, prior artifacts), with ingestion status per file.
...
- **Review ADR (Multi-Agent)** — joint ADR, Requirement, and Theory of Mind review of an existing decision.
...
- **Open in Canvas** — the rich structured editing surface (editing section by section, inline Socratic elicitation, AI Assist) as an alternative to the read oriented view panels (View ADR, View SOW, etc).
...
There is no free form, standing chat surface on the CLI side of this project, the CLI is a structured wizard. The chat panel and its automatic Socratic follow up suggestions only exist in the extension for now. The Dashboard's Questions tab only reflects Socratic probes from panels currently open, not a persisted history across sessions.
